...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'treat') {
      ...
      if (Math.random() < 0.1) { // 10% chance the treat is lost
        return interaction.reply({
...
if (Math.random() < 0.3) { // 30% chance of getting caught
  return interaction.reply({
    content: '❌ You were caught trying to play a trick! No effect, but the treat is spent.',
    ephemeral: true,
  });
}
...
// Fetch SpookyStats for valid members and filter out those with active immunity
const validTargets = await Promise.all(
  eligibleMembers.map(async (member) => {
    const memberStat = await SpookyStat.findOne({ where: { userId: member.id } });
    ...
    return !isImmune ? member : null; // Exclude immune members
  })
);

// Remove null values from the list of valid targets
const filteredTargets = validTargets.filter((member) => member);
...
const randomMember = filteredTargets[Math.floor(Math.random() * filteredTargets.length)];
const memberStat = await SpookyStat.findOne({ where: { userId: randomMember.id } });
      ...
      if (trickChance < 0.33) {
        ...
        const heistMembers = validTargets.slice(0, 3); // Limit to 3 targets
        const affectedUsers = [];
    
        for (const member of heistMembers) {
          const heistStat = await SpookyStat.findOne({ where: { userId: member.id } });
        ...
        console.log(`💰 ${user.username} performed a Great Heist on: ${affectedUsers.join(', ')}.`);
    
        return interaction.reply({
          content: `💰 ${user} stole treats from: ${affectedUsers.join(', ')}!`,
          ephemeral: false,
        });
        ...
        try {
          ...
          console.log(`🔄 ${user.username} reversed ${randomMember.user.username}'s nickname.`);
        ...
        console.log(`🔄 ${user.username} applied the Text Swap effect on ${randomMember.user.username}.`);
